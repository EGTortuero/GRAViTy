	...
	if Bootstrap == True:
		np.savetxt(	fname = ClassificationResultFile,
				X = np.column_stack((	list(map(', '.join, SeqIDLists_UcfVirus)),
							...
							list(map(lambda TaxoOfMaxSimScore, TaxoOfMaxSimScoreRange: ", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in list(zip(TaxoOfMaxSimScore, TaxoOfMaxSimScoreRange))]), list(zip(TaxoOfMaxSimScoreTable, TaxoOfMaxSimScoreRangeTable)))),
							list(map(lambda MaxSimScore, MaxSimScoreRange: ", ".join(["%s (%s)"%(Score, Range) for Score, Range in list(zip(MaxSimScore, MaxSimScoreRange))]), list(zip(np.around(MaxSimScoreTable,3).astype("str"), MaxSimScoreRangeTable)))),
							list(map(lambda PhyloStat, PhyloStatRange: ", ".join(["%s (%s)"%(Stat, Range) for Stat, Range in list(zip(PhyloStat, PhyloStatRange))]), list(zip(PhyloStatTable, PhyloStatRangeTable)))),
							list(map(lambda TaxoAssignment, TaxoAssignmentRange: ", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in list(zip(TaxoAssignment, TaxoAssignmentRange))]), list(zip(TaxoAssignmentTable, TaxoAssignmentRangeTable)))),
							list(map(lambda FinalisedTaxoAssignment, FinalisedTaxoAssignmentRange: "%s (%s)"%(FinalisedTaxoAssignment, FinalisedTaxoAssignmentRange), list(zip(FinalisedTaxoAssignmentList, FinalisedTaxoAssignmentRangeList)))),
							FinalisedVirusGroupingList,
							)),
				fmt = '%s',
				delimiter = "\t",
