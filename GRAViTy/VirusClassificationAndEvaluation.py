	...
	if Bootstrap == True:
		np.savetxt(	fname = ClassificationResultFile,
				X = np.column_stack((	list(map(', '.join, SeqIDLists_UcfVirus)),
							...
							list(map(lambda TaxoOfMaxSimScore: ", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in zip(TaxoOfMaxSimScore[0], TaxoOfMaxSimScore[1])]), zip(TaxoOfMaxSimScoreTable, TaxoOfMaxSimScoreRangeTable))),
							list(map(lambda MaxSimScore: ", ".join(["%s (%s)"%(Score, Range) for Score, Range in zip(MaxSimScore[0], MaxSimScore[1])]), zip(np.around(MaxSimScoreTable,3).astype("str"), MaxSimScoreRangeTable))),
							list(map(lambda PhyloStat: ", ".join(["%s (%s)"%(Stat, Range) for Stat, Range in zip(PhyloStat[0], PhyloStat[1])]), zip(PhyloStatTable, PhyloStatRangeTable))),
							list(map(lambda TaxoAssignment: ", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in zip(TaxoAssignment[0], TaxoAssignment[1])]), zip(TaxoAssignmentTable, TaxoAssignmentRangeTable))),
							list(map(lambda FinalisedTaxoAssignment: "%s (%s)"%(FinalisedTaxoAssignment[0], FinalisedTaxoAssignment[1]), zip(FinalisedTaxoAssignmentList, FinalisedTaxoAssignmentRangeList))),
							FinalisedVirusGroupingList,
							)),
				fmt = '%s',
				delimiter = "\t",
